 ...
 * 通用token解析器
 *
 * 代码看起来长，但就一个 {@code #parse(String text)} 方法，循环( 因为可能不只一个 )，解析以 openToken 开始，以 closeToken 结束的 Token ，并提交给 handler 进行处理，即 <x> 处。
 * 所以所以所以，可以耐心看下这段逻辑，也可以忽略，大体理解就好。
 * 关于 handler 这个 {@link TokenHandler} ，是一个接口。
