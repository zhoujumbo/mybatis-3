...
// <1.3> 如果找不到
  ...
  public void register(Class<?> typeHandlerClass) {
    boolean mappedTypeFound = false;
    // <3> 获得 @MappedTypes 注解，调用 #register(Class<?> javaTypeClass, Class<?> typeHandlerClass) 方法，注册指定 Java Type 的指定 TypeHandler 类
    ...
    // <4> 未使用 @MappedTypes 注解，则直接注册，调用 ② #register(TypeHandler<T> typeHandler) 方法，未使用 @MappedTypes 注解，调用 #register(TypeHandler<T> typeHandler) 方法，注册 TypeHandler 对象
    if (!mappedTypeFound) {
  ...
  public <T> TypeHandler<T> getInstance(Class<?> javaTypeClass, Class<?> typeHandlerClass) {
    // <1>获得 Class 类型的构造方法，适合 「EnumTypeHandler」 的情况。
    ...
    // <2> 获得空参的构造方法，适合 「IntegerTypeHandler」 的情况
    try {
